...
const REVEALS: { kind: SecretReveal['kind']; label: string; emoji: string }[] = [
...
function revealLabel(kind: SecretReveal['kind']): string {
...
function isIncomplete(secret: Secret, assets: readonly AssetRef[]): boolean {
  if (secret.reveal.kind === 'led') return false;
        ...
        <div className="secret-row" key={s.id}>
          ...
          >
            <span
              ...
              style={{ background: keys[s.trigger.key]?.appearance.baseColor }}
              aria-hidden
            />
            <span className="secret-row-text">
              <span className="secret-row-when">
                {t('secret.rowSummary', {
                  key: s.trigger.key + 1,
                  count: s.trigger.count,
                })}
              </span>
...
function SecretEditor({
  ...
  const setTrigger = (patch: Partial<Secret['trigger']>) => {
    onPatch({ trigger: { ...secret.trigger, ...patch } });
  };

  const setReveal = (kind: SecretReveal['kind']) => {
        ...
        <div className="cap-colors" role="group" aria-label={t('secret.whichKey')}>
          {keys.map((k) => (
            <button
              key={k.idx}
              type="button"
              className={`cap-swatch ${secret.trigger.key === k.idx ? 'on' : ''}`}
              style={{ background: k.appearance.baseColor }}
              aria-label={t('secret.keyAria', { n: k.idx + 1 })}
              aria-pressed={secret.trigger.key === k.idx}
              onClick={() => setTrigger({ key: k.idx as KeyIndex })}
            />
          ))}
        </div>
      </section>
        ...
        <div className="chip-grid">
          {COUNTS.map((n) => (
            <button
              key={n}
              type="button"
              className={`chip ${secret.trigger.count === n ? 'on' : ''}`}
              onClick={() => setTrigger({ count: n })}
            >
              {t('secret.times', { n })}
            </button>
          ))}
