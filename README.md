...
> 위 목록은 **v1 시점의 기록**이다. 그 뒤로 비밀 반응은 v1.5에, 흔적은 v1.6(고양이
> 발자국 한 종)과 v2.5(별·꽃·직접 그린 스탬프 + 사라짐/쌓임/걸어감)에, 재질 8종은
> v2.5에 들어왔다. `docs/roadmap_temp.md` 7번을 보라.
>
> 그때의 판단은 결과로도 맞았다: 값이 확정된 enum을 미리 넣어 둔 덕에 세 기능 모두
> **schemaVersion 변경도 마이그레이션도 없이** 들어왔다. 반대로 미뤄 뒀던
> `trace`의 방향·수명은 v2.5에서 `behavior` 하나로 정리됐는데, v1에 빈 껍데기를
> 넣었다면 지금 그 자리에 안 쓰는 필드 세 개가 남아 있었을 것이다.
